    ...
    <div className="mt-4">
      ...
      <div className="flex gap-3">
        {bench.map((b, i) => (
          ...
          >
            <Jersey team={b.team} number={b.id % 99 || 9} />
            <div className="text-sm">{b.name}</div>
            <div className="text-xs text-white/60">£{b.price.toFixed(1)}m</div>
          </div>
        ))}
      </div>
    </div>
...
export default function TeamGrid() {
  ...
  const takeNextIndex = (pos: Slot['pos']) => {
