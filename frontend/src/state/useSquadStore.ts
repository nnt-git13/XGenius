...
type State = {
  ...
  formation: string;
  budget: number;
  squad: Slot[];      // ordered GK, DEFs, MIDs, FWDs
  bench: Player[];
  benchSize: number;
  summary: Summary;
  ...
  /** Smart change (preserves players, moves overflow to bench if space). */
...
export const useSquadStore = create<State>()(
  persist(
    (set, get) => ({
      ...
      // Hard reset (for a "Reset XI" button)
      setFormation: (formation) => set({ formation, squad: mkEmptyXI(formation) }),

      // Smart formation change (preserve players by position; overflow → bench if room)
    ...
    // bump key if you had stale persisted shapes
    { name: 'xgenius-squad-v2' },
  ),
);
